...
The [get_black_border_percentage()](./notebooks/src/preprocessing.py) function originally failed to account for the corners of the image. The function itself has been fixed, but the model has been trained on images selected using the bugged version of this function.
...
│   └── src                                 <- Subfolder acting as Python module holding submodules with functions
│       ├── model_analysis
│       └── modeling
├── data                                    <- Folder containing external and code-generated data
│   ├── raw                                 <- Subfolder containing raw data from web scraping and generated by code
│   │   ├── actual_owls
│   │   │   ├── grayscale
│   │   │   ├── inverted
│   │   │   ├── named
...
│   │       ├── autopainter
│   │       └── pix2pix
├── logs                                    <- Folder containing logs from training and evaluation processes
│   ├── autopainter
│   └── pix2pix
├── presentation                            <- Folder containing PDF of presentation
└── visualizations                          <- Folder containing images and LaTeX code
```
</details>

<details>
  <summary>
    Minimum Google Drive Structure
  </summary>
  
```
├── src                                     <- Folder containing Python module holding submodules with functions
│   ├── model_analysis
│   └── modeling
├── data                                    <- Folder containing external and code-generated data
│   ├── test                                <- Subfolder containing test sketch/image pairs
...
│   │       ├── autopainter
│   │       └── pix2pix
├── models                                  <- Folder to place model weights from training
│   ├── autopainter
│   └── pix2pix
└── logs                                    <- Folder to place logs from training and evaluation processes
    ├── autopainter
    └── pix2pix
```
</details>
...
In order to remove duplicates, I used the [difPy package](https://github.com/elisemercury/Duplicate-Image-Finder) to select the highest resolution versions of each image. Finally, after choosing which version of each image to use and generating sketches, the images and sketches were resized to 256x256 with zero-padding to preserve aspect ratio. The sketches and images were then concatenated together horizontally, as specified in the [Tensorflow v2.x tutorial for pix2pix](https://www.tensorflow.org/tutorials/generative/pix2pix), for use in preprocessing for modeling.
